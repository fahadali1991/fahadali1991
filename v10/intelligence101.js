...
function detectSection(t){const x=norm(t),explicit=x.match(/(?:شعبه|الشعبه)\s*([ابجدهو]|\d{1,2})(?=\s|$)/);if(explicit)return explicit[1];const afterGrade=x.match(/(?:ال)?(?:اول|ثاني|ثالث|رابع|خامس|سادس)\s+(?:ال)?(?:ابتدائي|ابتدايي|متوسط|ثانوي)\s+([ابجدهو]|\d{1,2})(?=\s|$)/);return afterGrade?afterGrade[1]:'';}
