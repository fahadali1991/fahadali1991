const norm=s=>String(s||'').normalize('NFKC').replace(/[ًٌٍَُِّْـ]/g,'').replace(/[أإآٱ]/g,'ا').replace(/ى/g,'ي').replace(/ة/g,'ه').replace(/[،,؛;:!?؟.()\[\]{}"'«»]/g,' ').replace(/\s+/g,' ').trim().toLowerCase();
...
const SUBJECTS=[
{id:'quran',name:'القرآن الكريم والدراسات الإسلامية',icon:'🕌',keys:['القرآن','القران','سورة','سوره','آية','ايه','آيات','ايات','تلاوة','تلاوه','تجويد','تفسير','حديث','فقه','توحيد','عقيدة','عقيده','دراسات اسلامية','دراسات إسلامية','حفظ القرآن','حفظ القران']},
{id:'arabic',name:'اللغة العربية',icon:'📖',keys:['اللغة العربية','اللغه العربيه','لغتي','فهم قرائي','الفهم القرائي','قراءة','قراءه','إملاء','املاء','نحو','تعبير','خط عربي','تحسين الخط','الخط والكتابة','الخط والكتابه','كتابة','كتابه']},
...
function detectFamily(t){
 const rules=[
  ...
  {type:'تحليل نتائج',keys:['تحليل نتائج','حللت نتائج','حللنا نتائج','حللت نتايج','طلعت نتائج','طلعت نتايج','نتائج الاختبار','نتايج الاختبار','ظهر ضعف','قارنت مستوى','قارنتها بالعام السابق']},
...
function detectStageGrade(t){let stage='';if(any(t,['ابتدائي','الابتدائي']))stage='ابتدائي';else if(any(t,['متوسط','المتوسط']))stage='متوسط';else if(any(t,['ثانوي','الثانوي']))stage='ثانوي';const map=[['الأول',['الأول','الاول','اول','أول']],['الثاني',['الثاني','ثاني']],['الثالث',['الثالث','ثالث']],['الرابع',['الرابع','رابع']],['الخامس',['الخامس','خامس']],['السادس',['السادس','سادس']]];let grade='';for(const [g,vs] of map)if(any(t,vs)){grade=g;break}return{stage,grades:grade?[grade]:[]};}
...
function detectCount(t){const m=norm(t).match(/(?:عندي|لدي)?\s*(\d+)\s*(?:طلاب|طالب|طالبات)/);return m?Number(m[1]):null;}
