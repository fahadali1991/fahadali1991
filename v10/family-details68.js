...
export function familyDetailsPage(s){
 const f=FAMILIES[s.classification.type];
 let qs=f?.questions||[];
 if(s.classification.type==='برنامج / فعالية')qs=qs.filter(q=>!['product','follow'].includes(q.id));
 const suggest=inferFamilyDetails(s),duration=durationBlock(s);
