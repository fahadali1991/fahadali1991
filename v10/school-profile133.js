...
export function normalizeSchoolProfile133(value={}){const out={};for(const key of FIELDS)out[key]=clean(value?.[key]);return out}
export function schoolProfileComplete133(value={}){const p=normalizeSchoolProfile133(value);return Boolean(p.schoolName&&p.educationOffice&&p.academicYear)}
export function mergeSchoolProfile133(state,profile,{onlyMissing=true}={}){if(!state)return state;state.metadata=state.metadata||{};const p=normalizeSchoolProfile133(profile);for(const key of FIELDS){if(!p[key])continue;if(!onlyMissing||!clean(state.metadata[key]))state.metadata[key]=p[key]}return state}

let cache=null,loading=null;
export async function loadSchoolProfile133(){if(cache)return normalizeSchoolProfile133(cache);if(typeof indexedDB==='undefined')return normalizeSchoolProfile133({});if(!loading)loading=metaGet133(PROFILE_KEY).then(v=>{cache=normalizeSchoolProfile133(v||{});return cache}).catch(()=>normalizeSchoolProfile133({}));return normalizeSchoolProfile133(await loading)}
export async function saveSchoolProfile133(value={}){const p=normalizeSchoolProfile133(value);cache=p;if(typeof indexedDB!=='undefined')await metaSet133(PROFILE_KEY,p);return p}
...
function decoratePanel133(host){
 ...
 const form=host.querySelector('.formGrid');if(form){const bar=document.createElement('div');bar.className='schoolProfileBar133';bar.innerHTML='<div><b data-school-profile-summary133></b><span data-school-profile-status133></span></div><button type="button" class="linkBtn" data-school-profile-toggle133>تعديل البيانات</button>';form.before(bar);const year=form.querySelector('#academicYear')?.closest('label');if(year){const s=year.querySelector('span');if(s)s.innerHTML='العام الدراسي <b>(يظهر في التقرير)</b>'}const principal=form.querySelector('#principalName')?.closest('label');if(principal){const s=principal.querySelector('span');if(s)s.innerHTML='مدير المدرسة <small>(اختياري للاعتماد)</small>'}}
...
async function hydratePanel133(host){
 ...
 host.dataset.profileApplying133='1';try{for(const key of FIELDS)setPanelField133(host,key,profile[key])}finally{delete host.dataset.profileApplying133}
 const current=fromPanel133(host);if(Object.values(current).some(Boolean))saveSchoolProfile133(current).catch(()=>{});updateSummary133(host);host.dataset.profileReady133='1';
}
function scan133(){if(typeof document==='undefined')return;document.querySelectorAll('.analysisSchoolInfo122').forEach(host=>{if(host.dataset.profileReady133==='1'||host.dataset.profileHydrating133==='1')return;host.dataset.profileHydrating133='1';hydratePanel133(host).finally(()=>{if(host.isConnected)delete host.dataset.profileHydrating133})})}
