import {getMeta109,setMeta109} from './document-repository106.js?v=133';

const PROFILE_KEY='school-profile-v133';
const FIELDS=['schoolName','educationOffice','academicYear','principalName'];
const clean=v=>String(v??'').replace(/\s+/g,' ').trim();

export function normalizeSchoolProfile133(value={}){
 const out={};
 for(const key of FIELDS)out[key]=clean(value?.[key]);
 return out;
}
export function schoolProfileComplete133(value={}){
 const p=normalizeSchoolProfile133(value);
 return Boolean(p.schoolName&&p.educationOffice&&p.academicYear);
}
export function mergeSchoolProfile133(state,profile,{onlyMissing=true}={}){
 if(!state)return state;
 state.metadata=state.metadata||{};
 const p=normalizeSchoolProfile133(profile);
 for(const key of FIELDS){
  if(!p[key])continue;
  if(!onlyMissing||!clean(state.metadata[key]))state.metadata[key]=p[key];
 }
 return state;
}
...
export async function loadSchoolProfile133(){
 ...
 if(!loading)loading=getMeta109(PROFILE_KEY).then(v=>{cache=normalizeSchoolProfile133(v||{});return cache}).catch(()=>normalizeSchoolProfile133({}));
 return normalizeSchoolProfile133(await loading);
}
export async function saveSchoolProfile133(value={}){
 const p=normalizeSchoolProfile133(value);cache=p;
 if(typeof indexedDB!=='undefined')await setMeta109(PROFILE_KEY,p);
 return p;
}

function fromPanel133(host){
 const p={};for(const key of FIELDS)p[key]=host.querySelector(`#${key}`)?.value||'';
 return normalizeSchoolProfile133(p);
}
function setPanelField133(host,key,value){
 const el=host.querySelector(`#${key}`);if(!el||clean(el.value)||!value)return false;
 el.value=value;el.dispatchEvent(new Event('input',{bubbles:true}));return true;
}
...
function decoratePanel133(host){
 ...
 const form=host.querySelector('.formGrid');
 if(form){
  const bar=document.createElement('div');bar.className='schoolProfileBar133';bar.innerHTML='<div><b data-school-profile-summary133></b><span data-school-profile-status133></span></div><button type="button" class="linkBtn" data-school-profile-toggle133>تعديل البيانات</button>';
  form.before(bar);
...
async function hydratePanel133(host){
 decoratePanel133(host);
 const profile=await loadSchoolProfile133();
 if(!host.isConnected)return;
 for(const key of FIELDS)setPanelField133(host,key,profile[key]);
 const current=fromPanel133(host);if(Object.values(current).some(Boolean))saveSchoolProfile133(current).catch(()=>{});
 updateSummary133(host);
}
function scan133(){
 if(typeof document==='undefined')return;
 document.querySelectorAll('.analysisSchoolInfo122').forEach(host=>{if(host.dataset.profileHydrating133==='1')return;host.dataset.profileHydrating133='1';hydratePanel133(host).finally(()=>{if(host.isConnected)delete host.dataset.profileHydrating133})});
}

let saveTimer=null;
if(typeof document!=='undefined'){
 document.addEventListener('input',e=>{
  const host=e.target.closest?.('.analysisSchoolInfo122');if(!host||!FIELDS.includes(e.target.id))return;
  host.dataset.profileTouched133='1';updateSummary133(host);clearTimeout(saveTimer);saveTimer=setTimeout(()=>saveSchoolProfile133(fromPanel133(host)).catch(()=>{}),220);
 },true);
 document.addEventListener('click',e=>{
  const b=e.target.closest?.('[data-school-profile-toggle133]');if(!b)return;
  const host=b.closest('.analysisSchoolInfo122'),form=host?.querySelector('.formGrid');if(!form)return;
  host.dataset.profileTouched133='1';form.hidden=!form.hidden;host.dataset.profileCollapsed133=form.hidden?'1':'0';b.textContent=form.hidden?'تعديل البيانات':'إخفاء التفاصيل';
 },true);
 new MutationObserver(()=>queueMicrotask(scan133)).observe(document.documentElement,{subtree:true,childList:true});
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',scan133,{once:true});else scan133();
