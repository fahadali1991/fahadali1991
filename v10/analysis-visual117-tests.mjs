...
const base='http://127.0.0.1:4173/home106.html?v=118';
...
try{
 ...
 await fs.mkdir('artifacts',{recursive:true});await page.screenshot({path:'artifacts/analysis-preview-v118.png',fullPage:true});
 ...
 const pdf=await page.pdf({format:'A4',printBackground:true,preferCSSPageSize:true,path:'artifacts/analysis-print-v118.pdf'});const pageCount=(pdf.toString('latin1').match(/\/Type\s*\/Page\b/g)||[]).length;assert.equal(pageCount,1,`Chromium generated ${pageCount} PDF pages instead of 1`);
 assert.equal(errors.length,0,`browser errors: ${errors.join(' | ')}`);
 console.log(`V118 visual acceptance PASS: section beside grade, order=${seen.join(' > ')}, expected count beside max score, logo loaded, native share only, one-page PDF.`);
