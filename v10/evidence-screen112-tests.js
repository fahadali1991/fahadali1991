...
const analysis=state('تحليل نتائج','حللت نتائج الاختبارات وحددت فجوة الأداء وأسبابها',{metadata:{analysis:{maxScore:'20',masteryPercent:'80',scores:[12,18,19],names:['أحمد','محمد','علي']},familyDetails:{subject94:'اللغة العربية'}}});
...
for(const text of ['الشواهد الموجودة الآن','الشواهد المناسبة لهذا العمل','ما الذي يقوّي هذه الوثيقة؟','شواهد مرجعية إضافية','مرجع: جدول حصر الشواهد'])assert.ok(aHtml.includes(text),`evidence screen missing ${text}`);
...
assert.ok(aHtml.includes('لا تعتبر الخطة أو المتابعة أثرًا دون تنفيذ وقياس فعلي'),'analysis evidence chain must preserve the effect-quality rule');
...
console.log('V112/V117 evidence screen PASS: current evidence, Analysis strengthening chain, source-grounded suggestions and no false impact/indicator claim.');
