...
const PLAN=[...COMMON,section('basis','أساس الخطة',{required:true,layout:'card',priority:9}),section('goal','الهدف',{required:true,layout:'goal-box',priority:10}),section('procedures','الإجراءات',{required:true,layout:'procedure-table',priority:10}),section('owner','المسؤول',{when:s=>has(s,'owner'),layout:'responsibility',priority:8}),section('schedule','الزمن',{layout:'timeline',priority:8}),section('measurement','القياس',{when:hasMeasure,layout:'measure',priority:9}),section('follow','المتابعة',{when:s=>has(s,'follow'),layout:'status',priority:9}),...TAIL];
const FOLLOW=[...COMMON,section('subject','محور المتابعة',{required:true,layout:'card',priority:9}),section('previous','الوضع السابق',{layout:'before',priority:7}),section('current','الوضع الحالي',{required:true,layout:'now',priority:9}),section('action','الإجراء',{when:s=>has(s,'action'),layout:'action',priority:9}),section('next','الخطوة التالية',{when:s=>has(s,'follow'),layout:'next',priority:9}),...TAIL];
