...
test('analysis derives finding and separates action status',()=>{const s=baseState('تحليل نتائج');const m=matrix([q('basis'),q('finding'),q('cause'),q('action'),q('follow')]);assert.deepEqual(routeSequence106(s,m).slice(0,5),['basis','cause','actionStatus','action','follow'])});
