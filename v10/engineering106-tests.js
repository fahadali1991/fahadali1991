...
test('analysis derives finding and defers interpretation until after results',()=>{const s=baseState('تحليل نتائج');const m=matrix([q('basis'),q('finding'),q('cause'),q('action'),q('follow')]);const seq=routeSequence106(s,m);assert.deepEqual(seq,['basis']);assert.ok(!seq.includes('finding'),'finding is derived, never asked');for(const id of ['cause','actionStatus','action','follow'])assert.ok(!seq.includes(id),`${id} must not be asked before the Analysis result`)});
