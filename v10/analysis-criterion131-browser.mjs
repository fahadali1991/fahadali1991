...
try{
 for(const [label,viewport] of cases){
  ...
  assert.match(criterionLabel,/مستوى الإتقان المستهدف/);
  ...
  assert.match(noCriterionState.finding,/لم يحدد المعلم مستوى الإتقان المستهدف/);
  ...
  assert.match(neutral.plans,/لا توجد خطة آلية دون مستوى إتقان مستهدف/);
  ...
  assert.equal(Number(explicit.mastery),80,`${label}: explicit target must persist`);
 ...
 console.log('V131 browser acceptance PASS: optional target level stays blank until teacher defines it on mobile + desktop.');
