...
export function guideSupportSentence76(s,limit=1){const links=strongGuideLinks76(s).slice(0,limit);if(!links.length)return'';if(links.length===1){const x=links[0];return `ويرتبط العمل دلاليًا بممارسة «${String(x.text||'').replace(/\.$/,'')}» ضمن مجال ${x.domainName}.`;}return `ويرتبط العمل دلاليًا بممارسات ضمن ${[...new Set(links.map(x=>`مجال ${x.domainName}`))].join(' و')}.`}
