...
function stageAdj(s){const x=n(s.stage||'');if(x.includes('ابتدائي'))return'الابتدائي';if(x.includes('متوسط'))return'المتوسط';if(x.includes('ثانوي'))return'الثانوي';const g=n((s.grades||[]).join(' '));if(g.includes('ابتدائي'))return'الابتدائي';if(g.includes('متوسط'))return'المتوسط';if(g.includes('ثانوي'))return'الثانوي';return''}
function stageNoun(s){const x=stageAdj(s);if(x==='الابتدائي')return'المرحلة الابتدائية';if(x==='المتوسط')return'المرحلة المتوسطة';if(x==='الثانوي')return'المرحلة الثانوية';return''}
function gradePhrase(s){const gs=uniq((s.grades||[]).map(gradeOrder).filter(Boolean));const st=stageAdj(s);if(!gs.length)return'';if(gs.length===1)return`طلاب الصف ${gs[0]} ${st}`;if(gs.length===2)return`طلاب الصفين ${gs[0]} و${gs[1]} ${st}`;return`طلاب الصفوف ${joinAr(gs)} ${st}`}
function audiencePhrase(s){if((s.audiences||[]).includes('الطلاب'))return gradePhrase(s)||(stageNoun(s)?`طلاب ${stageNoun(s)}`:'الطلاب');if((s.audiences||[]).includes('المعلمون'))return'المعلمين';if((s.audiences||[]).includes('أولياء الأمور'))return'أولياء الأمور';if((s.audiences||[]).includes('الإداريون'))return'الإداريين';return''}
...
export function extractTitleFacts62(s){return{
 family:s.classification?.type||'',subtype:s.classification?.subtype||'',topicConcept:topicConcept(s),target:audiencePhrase(s),stage:stageAdj(s),grades:uniq((s.grades||[]).map(gradeOrder).filter(Boolean)),assessmentSource:assessmentSource(s.raw||'')
}}
...
function programTitles(f){if(!f.topicConcept)return[];const kind=f.subtype||'برنامج';const short=`${kind} ${f.topicConcept}`;const formal=f.target?`${kind} ${f.topicConcept} ل${f.target}`:short;const detailed=f.target?`${short} لدى ${f.target}`:'';return[formal,short,detailed]}
