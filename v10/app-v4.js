...
import {landing,entryForm,understanding,goalsPage} from './renderers.js?v=4';
import {descriptionChoicesPage,descriptionEditorPage} from './description-ui-v4.js';
import {descriptionVariants} from './description-variants-v4.js';
import {evidencePage} from './evidence-ui.js?v=4';
import {finalDocument} from './final-ui-v4.js';
...
const home=()=>{state=null;entryIntent='smart';render(landing(),true)};
const info=(top=true)=>render(understanding(state),top);
const goals=(top=true)=>render(goalsPage(state),top);
const descChoices=()=>{sync();state.metadata.generatedDescription='';render(descriptionChoicesPage(state),true)};
const descEditor=()=>render(descriptionEditorPage(state),true);
const evidence=(top=true)=>{sync();render(evidencePage(state),top)};
...
app.addEventListener('click',e=>{
 ...
 const dv=e.target.closest('[data-description-variant]');if(dv&&state){sync();const v=descriptionVariants(state).find(x=>x.id===dv.dataset.descriptionVariant);if(v){state.metadata.descriptionVariant=v.id;state.metadata.descriptionVariantLabel=v.label;state.metadata.generatedDescription=v.text;descEditor()}return}
 ...
 switch(b.dataset.action){
  ...
  case'go-description':descChoices();break;
  case'description-choices':descChoices();break;
  case'edit-description':descEditor();break;
  case'go-evidence':sync();evidence(true);break;
  ...
  case'finalize':sync();render(finalDocument(state),true);break;
 }
});
home();
