...
export function analysisJourneyPanel147(state){
 const a=analysisDocumentAvailability147(state);
 // Analysis already owns the integrated "what to add" selector in analysisOutputPanel134.
 // The journey panel is for standalone Classification/Remedial/Enrichment so the user can continue without re-entry.
 if(a.current==='analysis'||!a.next.length)return'';
...
function rerender147(){
 ...
 const journey=document.querySelector('[data-analysis-journey147]');
 const next=analysisJourneyPanel147(lastState);
 if(journey){if(next)journey.outerHTML=next;else journey.remove()}
 else if(next){document.querySelector('[data-analysis-output-host134]')?.insertAdjacentHTML('afterend',next)}
}
