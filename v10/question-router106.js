...
export function routeNextQuestion106(state,matrix){const ctx=buildCanonicalContext106(state),index=byId(matrix),f=family(ctx),order=ORDER[f]||['reason','method','participation'];for(const id of order){if(resolved(ctx,id,matrix,state))continue;let q=index[id]||extras(id,f);if(f==='تحليل نتائج'&&id==='action'){const status=ctx.details.actionStatus?.value;if(status==='لم يحدد بعد')continue;if(q){const prefix=status==='نُفذ فعلًا'?'تم التنفيذ: ':'مخطط: ';q={...q,q:status==='نُفذ فعلًا'?'ما الإجراء الذي نُفذ فعلًا بعد التحليل؟':'ما الإجراء المخطط لتنفيذه بعد التحليل؟',help:status==='نُفذ فعلًا'?'اختر فقط ما تم تنفيذه فعليًا.':'اختر ما ستنفذه؛ سيظهر في الوثيقة بصفته مخططًا لا منجزًا.',opts:(q.opts||[]).map(x=>prefix+x)}}}if(q)return{question:q,gap:id,done:false,context:ctx}}if(f!=='تحليل نتائج'&&makesOutcomeClaim(ctx)&&!has(ctx,'measurement'))return{question:measurementQuestion(),gap:'measurement',done:false,context:ctx};return{question:null,gap:'',done:true,context:ctx}}
