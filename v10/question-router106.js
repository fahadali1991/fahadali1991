...
function resolved(ctx,id,matrix){if(id==='skillFocus')return Boolean(ctx.education.skill.value)||!(matrix.questions||[]).some(q=>q.id==='skillFocus');return has(ctx,answerKey(id))}
...
function measurementQuestion(){return{id:'measurement',q:'كيف تحققت من هذه النتيجة؟',help:'اختر أداة قياس حقيقية. وإذا لم يتم القياس بعد فصرّح بذلك.',opts:['اختبار أو تقويم قصير','مقارنة قبل وبعد','ملاحظة أداء مباشرة','منتج أو عمل من المستفيدين','سجل متابعة','استبانة أو تغذية راجعة','لم يتم القياس بعد'],max:2,kind:'MeasuredResult'}}
export function routeNextQuestion106(state,matrix){const ctx=buildCanonicalContext106(state),index=byId(matrix),f=family(ctx),order=ORDER[f]||['reason','method','participation'];for(const id of order){if(resolved(ctx,id,matrix))continue;const q=index[id]||extras(id,f);if(q)return{question:q,gap:id,done:false,context:ctx}}if(makesOutcomeClaim(ctx)&&!has(ctx,'measurement'))return{question:measurementQuestion(),gap:'measurement',done:false,context:ctx};return{question:null,gap:'',done:true,context:ctx}}
