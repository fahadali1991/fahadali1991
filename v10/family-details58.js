...
function durationBlock(s){
 ...
 return `<div class="chooser durationContext"><small>${esc(spec.label)}</small>${auto?`<p class="questionHelp"><b>فهم المحرك من وصفك:</b> ${esc(selected)}. عدله فقط إذا احتجت.</p>`:'<p class="questionHelp">اختر الأقرب، أو اتركه دون تحديد إذا لم يكن مهمًا.</p>'}<div class="chips choiceGrid">${spec.options.map(x=>`<button type="button" class="chip ${selected===x?'on':''}" data-duration-pick="${esc(x)}">${esc(x)}</button>`).join('')}</div>${selected==='أخرى'?`<label class="fullField"><span>اكتب ${esc(spec.label)}</span><input id="customDuration" value="${esc(s.metadata?.customDuration||'')}"></label>`:''}</div>`;
}
