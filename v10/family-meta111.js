...
const DEFINITIONS111={
 ...
 'تحليل نتائج':[
  ...
  {id:'expectedCount',label:'عدد الطلاب المتوقع',q:'كم عدد طلاب الفصل؟ (اختياري)',type:'number',placeholder:'مثال: 20',optional:true,help:'اكتب العدد إذا أردت أن يتحقق النظام من اكتمال الدرجات. يمكنك تجاوزه.'},
  {id:'section',label:'الشعبة',q:'ما الشعبة أو رقم الفصل؟ (اختياري)',type:'text',placeholder:'مثال: أ أو 1 أو 2',optional:true,help:'يظهر بجانب الصف عند إدخاله، ويمكن تجاوزه إذا لم تحتجه.'}
 ],
 'خطة':[
...
export function familyMetaQuestion111(state){const def=nextFamilyMeta111(state);if(!def)return'';const value=String(meta(state)[def.id]??''),help=def.help||'اخترها بلمسة واحدة. استخدم الخيار الآخر فقط إذا لم ينطبق الفصلان المعتادان.';return `<div class="familyMeta111" data-family-meta-host111 data-field111="${esc(def.id)}"><div class="adaptiveKicker106">${def.optional?'بيانات اختيارية':'بيانات ستظهر في الوثيقة'}</div><h2>${esc(def.q)}</h2><p class="questionHelp">${esc(help)}</p>${control(def,value)}<div class="row adaptiveActions106" style="margin-top:12px"><button type="button" class="btn primary" data-family-meta-next111 ${value?'':'disabled'}>التالي</button><button type="button" class="btn" data-family-meta-skip111>${def.optional?'تجاوز':'لا ينطبق / تجاوز'}</button></div></div>`}
...
function syncDerived111(){if(!current)return;current.metadata=current.metadata||{};current.metadata.familyDetails=current.metadata.familyDetails||{};if(current.classification?.type==='تحليل نتائج'){const m=meta(current),basis=[m.assessmentType,m.period].filter(Boolean).join(' — ');if(basis)current.metadata.familyDetails.basis=basis;if(m.section)current.metadata.section111=m.section;if(m.expectedCount)current.metadata.expectedCount111=String(m.expectedCount)}}
...
if(typeof document!=='undefined'){
 document.addEventListener('input',e=>{if(!current)return;const custom=e.target.closest?.('[data-period-custom-input114]');if(custom){const host=custom.closest('[data-family-meta-host111]'),el=host?.querySelector('[data-family-meta111]');if(el){el.value=(custom.value||'').trim();meta(current)[el.dataset.familyMeta111]=el.value;syncDerived111();const next=host.querySelector('[data-family-meta-next111]');if(next)next.disabled=!el.value}return}const el=e.target.closest?.('[data-family-meta111]');if(!el)return;meta(current)[el.dataset.familyMeta111]=(el.value||'').trim();syncDerived111();const host=el.closest('[data-family-meta-host111]');const next=host?.querySelector('[data-family-meta-next111]');if(next)next.disabled=!String(el.value||'').trim()},true);
 document.addEventListener('click',e=>{if(!current)return;const custom=e.target.closest('[data-period-custom114]');if(custom){const host=custom.closest('[data-family-meta-host111]');host?.querySelector('.periodCustom114')?.removeAttribute('hidden');host?.querySelector('[data-period-custom-input114]')?.focus();return}const choice=e.target.closest('[data-family-meta-choice111]');if(choice){const host=choice.closest('[data-family-meta-host111]'),el=host?.querySelector('[data-family-meta111]');if(el){el.value=choice.dataset.familyMetaChoice111;meta(current)[el.dataset.familyMeta111]=el.value;syncDerived111();host.querySelectorAll('[data-family-meta-choice111]').forEach(b=>b.classList.toggle('on',b===choice));host.querySelector('.periodCustom114')?.setAttribute('hidden','');const next=host.querySelector('[data-family-meta-next111]');if(next)next.disabled=false}return}const skip=e.target.closest('[data-family-meta-skip111]');if(skip){const id=skip.closest('[data-family-meta-host111]')?.dataset.field111;if(id)current.metadata.familyMeta111Skipped=[...new Set([...(current.metadata.familyMeta111Skipped||[]),id])];signalPersist();refresh();return}const next=e.target.closest('[data-family-meta-next111]');if(next&&!next.disabled){signalPersist();refresh()}},true);
}
if(typeof document!=='undefined'&&!document.getElementById('period-choice114-style')){const s=document.createElement('style');s.id='period-choice114-style';s.textContent='.periodChoice114{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;margin-top:10px}.periodTile114{border:1px solid #d9e6e1;background:#fff;border-radius:15px;padding:12px 7px;display:flex;flex-direction:column;align-items:center;gap:3px;color:#173f38;min-height:88px}.periodTile114 b{font-size:22px;color:#0da9a6}.periodTile114 span{font-weight:800;font-size:13px}.periodTile114 small{color:#788984;font-size:10px}.periodTile114.on{border:2px solid #07a869;background:#f1faf6;box-shadow:0 5px 18px rgba(7,168,105,.12)}.periodTile114.custom b{color:#3d7eb9}.periodCustom114{margin-top:10px}.periodCustom114 input{width:100%}@media(max-width:600px){.periodChoice114{grid-template-columns:repeat(2,minmax(0,1fr))}}';document.head.appendChild(s)}