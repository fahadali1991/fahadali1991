...
function core(s,level='medium'){
 const x=ctx(s),f=x.fd,a=[],tail=contextTail(s);
 if(x.t==='برنامج / فعالية'){
   a.push(`نُفذ ${x.title} لصالح ${x.a}${tail}.`);
   ...
   a.push(`عُقد ${x.title} بمشاركة ${x.a}${tail}.`);
   ...
   a.push(`أُجري ${x.title} لخدمة ${x.a}${durationPhrase(s)}.`);
   ...
   a.push(`أُعدت ${x.title} لخدمة ${x.a}${durationPhrase(s)}.`);
   ...
   a.push(`نُفذ ${x.title} لخدمة ${x.a}${durationPhrase(s)}.`);
   ...
   if(goal)a.push(sentence('وهدفت المتابعة إلى ',goal));
   if(method)a.push(sentence('واعتمدت المتابعة على ',method));
   if(level!=='short'&&action)a.push(sentence('وتم اتخاذ إجراء شمل ',action));
   if(level==='long'&&follow)a.push(sentence('وتستمر المتابعة من خلال ',follow));
 }else if(x.t==='تطوير مهني'){
   a.push(`نُفذ ${x.title} بما يخدم ${x.a}${tail}.`);
   ...
   if(need)a.push(sentence('وجاء النشاط استجابةً لـ',need));
   if(method)a.push(sentence('وتضمن التنفيذ ',method));
   if(level!=='short'&&application)a.push(sentence('وتركزت الاستفادة التطبيقية في ',application));
   if(level==='long'&&follow)a.push(sentence('وتتم متابعة أثر التطوير من خلال ',follow));
 }else a.push(`تم تنفيذ ${x.title} لصالح ${x.a}${tail}.`);
 return c(a.join(' '));
}
...
const LABELS={reason:'سبب التنفيذ: ',goal:'الهدف: ',method:'طريقة التنفيذ: ',participation:'ملاحظة أثناء التنفيذ: ',purpose:'غرض الاجتماع: ',work:'أبرز ما نوقش: ',product:'القرار أو المخرج: ',basis:'أساس العمل: ',finding:'أبرز ما ظهر: ',cause:'سبب محتمل: ',action:'الإجراء: ',follow:'المتابعة: ',need:'الاحتياج المهني: ',tool:'أداة المتابعة: ',content:'المحتوى: ',application:'التطبيق: ',impact:'الأثر: ',outcome:'الإجراء الناتج: '};
export function bulletText80(s){const x=ctx(s),a=[`تم تنفيذ ${x.title} لصالح ${x.a}.`],place=c(s.metadata?.place||s.metadata?.placeChoice||''),duration=c(s.metadata?.duration||s.metadata?.durationChoice||'');if(place)a.push(`مكان التنفيذ: ${place}.`);if(duration)a.push(`المدة: ${duration}.`);const entries=Object.entries(x.fd).filter(([,v])=>c(v));for(const [k,v] of entries.slice(0,5)){const label=LABELS[k]||'تفصيل: ';a.push(`${label}${c(v)}.`)}strongGuideLinks76(s).slice(0,2).forEach(z=>a.push(`يدعم العمل الممارسات المرتبطة بـ${String(z.text||'').replace(/\.$/,'')} ضمن مجال ${z.domainName}.`));return a.slice(0,8)}
