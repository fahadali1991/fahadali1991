...
export function analyze101(raw,entryIntent='smart'){
 ...
 if(entryIntent==='classification'){core.setType?.(s,'تحليل نتائج');core.setSubtype?.(s,'تحليل نتائج');s.metadata.directEntry128='classification'}
 if(entryIntent==='remedial'){core.setType?.(s,'خطة');core.setSubtype?.(s,'خطة علاجية');s.metadata.directEntry128='remedial'}
 if(entryIntent==='enrichment'){core.setType?.(s,'خطة');core.setSubtype?.(s,'خطة إثرائية');s.metadata.directEntry128='enrichment'}
