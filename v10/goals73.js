...
import {goalGroups65} from './goals65.js?v=73';
import {guideLinks73,strongGuideLinks73} from './guide-link73.js?v=73';
const clean=s=>String(s||'').replace(/\s+/g,' ').trim();
const uniq=a=>[...new Set((a||[]).map(clean).filter(Boolean))];
function indicatorGoals(i){const c=i.code;
 const M={
 ...
 '4-2-1-1':['رفع الوعي بمتطلبات الأمن والسلامة في البيئة المدرسية','تعزيز الالتزام بالممارسات الوقائية وإجراءات السلامة'],
 '4-2-1-2':['رفع جودة متابعة أعمال الصيانة للمبنى والمرافق والتجهيزات','دعم استمرارية جاهزية المرافق والتجهيزات للاستخدام'],
 '4-2-1-3':['تعزيز استدامة نظافة المبنى المدرسي ومرافقه','رفع مستوى المتابعة المنتظمة لممارسات النظافة']
 };if(M[c])return M[c];
 const t=i.text.replace(/^ت(ضع|تابع|طبق|نفذ|عزز|دعم|وفر|ظهر)|^ي(حقق|ظهر|شارك|لتزم)/,'').replace(/\.$/,'').trim();
 return t?[`دعم الممارسات المرتبطة بـ${t}`,`رفع جودة الأداء المرتبط بالمؤشر (${c})`]:[]}
export function goalGroups73(s){const base=goalGroups65(s),strong=strongGuideLinks73(s),smart=uniq(strong.flatMap(indicatorGoals));const all=uniq([...smart,...base.primary,...base.additional]);return{primary:all.slice(0,4),additional:all.slice(4,8)}}
function linkSummary(s){const all=guideLinks73(s),strong=all.filter(x=>x.strength==='قوي'),p=strong[0]||all[0];if(!p)return'';const note=p.strength==='قوي'?`ارتباط قوي بالمؤشر (${p.code})`:`ارتباط محتمل بالمؤشر (${p.code}) يحتاج إلى شاهد إضافي`;return `<div class="helperBox" style="margin-bottom:16px"><b>الارتباط الذي فهمه المحرك</b><p><strong>${esc(p.domainName)}</strong> — ${esc(note)}</p><p class="questionHelp">${esc(p.reason)}</p>${strong.length>1?`<p class="questionHelp">كما وجد ارتباطًا قويًا إضافيًا بالمؤشر (${esc(strong[1].code)}).</p>`:''}</div>`}
export function goalsPage73(s){const {primary,additional}=goalGroups73(s),selected=s.answers?.goals||[],other=Boolean(s.metadata?.otherGoalEnabled);const button=x=>`<button type="button" class="chip ${selected.includes(x)?'on':''}" data-goal="${esc(x)}">${esc(x)}</button>`;return `<section class="card"><button class="linkBtn" data-action="back-family-details">→ رجوع</button><div class="muted">الخطوة الثالثة</div><h1>الأهداف</h1><p class="lead">صاغ المحرك الأهداف من موضوع العمل والفئة، ثم حسّنها بالارتباطات القوية من القاموس الكامل للمؤشرات.</p>${linkSummary(s)}<div class="chips choiceGrid">${primary.map(button).join('')}</div>${additional.length?`<details style="margin-top:14px"><summary class="btn" style="cursor:pointer">عرض أهداف إضافية</summary><div class="chips choiceGrid" style="margin-top:12px">${additional.map(button).join('')}</div></details>`:''}<div class="chips choiceGrid" style="margin-top:12px"><button type="button" class="chip ${other?'on':''}" data-action="toggle-other-goal">أخرى</button></div>${other?`<label class="fullField"><span>اكتب الهدف</span><input id="otherGoal" value="${esc(s.metadata.otherGoal||'')}"></label>`:''}<div class="row"><button class="btn primary" data-action="go-description">التالي: العنوان</button></div></section>`}
