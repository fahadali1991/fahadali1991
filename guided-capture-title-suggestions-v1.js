...
function titleSubjectFromRaw(raw){
  ...
  if(m){
    let x=m[1].replace(/(?:لطلاب|للطالب|للمعلمين|للمعلم|لأولياء|لأولياء الأمور|عشان|بهدف|من أجل)\s.*$/i,'').trim();
    x=x.replace(/^(?:عن|حول)\s+/,'').trim();
    if(x.length>2&&!/^(?:تربوي|تربوية|تعليمي|تعليمية)$/.test(x))return x;
  }
  let n=norm(s);
  if(has(n,'خط'))return'تحسين الخط والمهارات الكتابية';
  if(has(n,'قراءه','قرائي','فهم قرائي'))return'تنمية مهارات القراءة والفهم القرائي';
  if(has(n,'غياب'))return'الحد من الغياب وتعزيز الانضباط';
  if(has(n,'تاخير','متاخر'))return'الحد من التأخر وتعزيز الانضباط';
  if(has(n,'نتائج','درجات','اختبار'))return'تحسين نتائج الطلاب والتحصيل الدراسي';
  if(has(n,'ذكاء اصطناعي'))return'توظيف الذكاء الاصطناعي في التعليم';
  if(has(n,'امن','سلامه','اخلاء'))return'تعزيز الأمن والسلامة المدرسية';
  if(has(n,'نظافه'))return'تعزيز النظافة والمسؤولية المدرسية';
  if(has(n,'قيم','وعي','توعيه'))return'تعزيز الوعي والقيم المدرسية';
  if(has(n,'وطنيه','وطن'))return'تعزيز القيم الوطنية والانتماء';
  return cur?.topic||'';
}
function genericTitle(x){
  x=norm(String(x||'').replace(/[\s\-–—]+/g,' ').trim());
  return /^(برنامج|برنامج تربوي|فعاليه|فعاليه تربويه|حمله|حمله تربويه|مبادره|مبادره تربويه|مسابقه|نشاط|نشاط تربوي|خطه|خطه تنفيذيه|ورشه|تدريب|اجتماع)$/.test(x);
}
function buildTitleSuggestions(f){
  ...
  if(existing&&!genericTitle(existing))a.push(existing);
  if(f.type==='برنامج / فعالية'){
    let k=st||'برنامج';
    if(subject){
      a.push(`${k} ${subject}`);
      if(has(norm(subject),'خط')){a.push(`${k} إتقان المهارات الكتابية`);a.push(`نحو خط أجمل وكتابة أكثر إتقانًا`)}
      else if(has(norm(subject),'قراءه','قرائي')){a.push(`${k} نحو قراءة أكثر إتقانًا`);a.push(`قارئ واعٍ`)}
      else if(has(norm(subject),'وعي','قيم')){a.push(`${k} الوعي والقيم في البيئة المدرسية`);a.push(`قيمنا سلوك`)}
      else if(has(norm(subject),'نظافه','مسؤوليه')){a.push(`${k} بيئتي مسؤوليتي`);a.push(`مدرستي أجمل`)}
      else {a.push(`${k} لتنمية ${subject.replace(/^تحسين\s+/,'')}`);a.push(`${k} نحو ${subject.replace(/^تعزيز\s+/,'')}`)}
    }
  }else if(f.type==='تطوير مهني'){
    if(subject){a.push(`${st||'نشاط تطوير مهني'} حول ${subject}`);a.push(`تطوير الممارسات المهنية في ${subject}`);a.push(`نحو ممارسة مهنية أكثر فاعلية في ${subject}`)}
  }else if(f.type==='إجراء متابعة'){
    if(has(f.n,'غياب')){a.push('متابعة الغياب وتعزيز الانضباط المدرسي');a.push('رصد الغياب المتكرر ودعم الانتظام');}
    if(has(f.n,'تاخير','متاخر')){a.push('متابعة حالات التأخر وتعزيز الانضباط');a.push('رصد التأخر ودعم الانتظام المدرسي');}
    ...
    if(subject)a.push(`اجتماع حول ${subject}`);
  }else if(f.type==='خطة'){
    let p=st||'خطة تنفيذية';
    if(subject)a.push(`${p} لتحسين ${subject.replace(/^تحسين\s+/,'')}`);
    if(has(f.n,'قراءه','قرائي'))a.push(`${p} لرفع مستوى الأداء القرائي`);
    if(has(f.n,'نتائج','درجات','اختبار'))a.push(`${p} لتحسين نتائج الطلاب`);
    if(has(f.n,'تطوير مهني','معلمين'))a.push(`${p} لتنمية الممارسات المهنية`);
  }
  return uniq(a.map(x=>x.replace(/\s+/g,' ').trim()).filter(x=>x.length>4&&!genericTitle(x))).slice(0,3);
}
function ensureTitleSuggestions(){
  ...
  if((!cur.workTitle||genericTitle(cur.workTitle))&&list.length)cur.workTitle=list[0];
  if(cur.workTitle&&!genericTitle(cur.workTitle)&&list.length&&!list.includes(cur.workTitle))cur._titleSuggestions=uniq([cur.workTitle,...list]).slice(0,3);
...
function titleSuggestionBox(){
  ensureTitleSuggestions();
  let list=cur?._titleSuggestions||[],idx=cur?._titleIndex||0,t=currentSuggestedTitle()||'اكتب اسم العمل';
...
if(_titleBaseUnderstanding){understandingView=function(){ensureTitleSuggestions();let h=_titleBaseUnderstanding();let re=/<label><span>اسم\/عنوان العمل<\/span><input[^>]*><\/label>/;if(re.test(h))h=h.replace(re,titleSuggestionBox());return h;}}
const _titleBaseSetSubtype=typeof setSubtype==='function'?setSubtype:null;
if(_titleBaseSetSubtype){setSubtype=function(v){_titleBaseSetSubtype(v);if(cur){cur.workTitle='';cur._titleIndex=0;cur._titleSuggestions=[];ensureTitleSuggestions();renderUnderstanding()}}}
const _titleBaseSetTypeEnhanced=typeof setTypeEnhanced==='function'?setTypeEnhanced:null;
if(_titleBaseSetTypeEnhanced){setTypeEnhanced=function(v){_titleBaseSetTypeEnhanced(v);if(cur){cur.workTitle='';cur._titleIndex=0;cur._titleSuggestions=[];ensureTitleSuggestions();renderUnderstanding()}}}
